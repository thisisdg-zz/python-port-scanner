...
def verify_checksum(param_dict, merchant_key, checksum, salt=None):
    # Remove checksum
    ...
    salt = salt if salt else paytm_hash[-4:]
...
def __encode__(to_encode, iv, key):
    ...
    # Encrypt
    c = AES.new(key, AES.MODE_CBC, iv.encode('latin-1'))
    to_encode = c.encrypt(to_encode.encode('latin-1'))
    # Encode
